        ...
        if (ws.kind === 'scratch') {
          ...
          if (!nodeFs.existsSync(claudeMd)) {
            ...
            nodeFs.writeFileSync(claudeMd, [
              ...
              'Before your first `workon`, state in the channel which repo(s) you',
              'will work in and proceed — ask first only if the task is ambiguous.',
              'The worktree is created on branch `slack/<channel>` from a fresh',
              'origin default; edit there by absolute path and commit.',
              '',
              '**Finish every repo task by opening a pull request** (`git push`,',
              'then `gh pr create`) and posting its URL in the channel — a pushed',
              'branch without a PR is unfinished work. Humans review and merge;',
              'GitHub deletes the merged branch automatically. When the task is',
              'done, say so and a human archives the channel — everything',
              'committed and pushed survives; worktrees are then verified and',
